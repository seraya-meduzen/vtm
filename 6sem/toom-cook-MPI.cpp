...
vector<string> matmul(vector<pair<string, string>> matrix, vector<string> vector_)
{
    ...
    int num_rows_per_process = matrix_size / size;
    int remainder_rows = matrix_size % size;
    ...
    vector<pair<string, string>> results_private(num_rows_per_process, make_pair("0", "1"));
    ...
    for (int y = start_row; y < end_row; y++)
    {
        for (int i = 0; i < matrix_size; i++)
        {
            ...
            auto num = add(toom_cook(tmp.first, results_private[y - start_row].second), toom_cook(tmp.second, results_private[y - start_row].first));
            auto denom = toom_cook(tmp.second, results_private[y - start_row].second);

            results_private[y - start_row].first = num;
            results_private[y - start_row].second = denom;
        }

        results[y] = division(results_private[y - start_row].first, stoi(results_private[y - start_row].second));
    }
    ...
    for (int i = 0; i < size; i++)
    {
        if (i == rank)
        {
            ...
            {
                MPI_Send(&results[j][0], results[j].size() + 1, MPI_CHAR, 0, j, MPI_COMM_WORLD);
            }
        }
        ...
        if (rank == 0)
        {
            ...
            if (i == size - 1)
            {
                num_rows = num_rows_per_process + remainder_rows;
            }
            else
            {
                num_rows = num_rows_per_process;
            }

            for (int j = 0; j < num_rows; j++)
            {
                char buffer[100];
                MPI_Recv(&buffer, 100, MPI_CHAR, i, j, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                results[i * num_rows_per_process + j] = string(buffer);
            }
        }
    }
