...
def main():
    ...
    with open('ORIENTATION.txt','wb') as f:
        for line in np.matrix(odom[ORIENTATION]):
            np.savetxt(f, line, fmt='%.2f')
    with open('LINEAR_TWIST.txt','wb') as f:
        for line in np.matrix(odom[LINEAR_T]):
            np.savetxt(f, line, fmt='%.2f')
    with open('ANGULAR_TWIST.txt','wb') as f:
        for line in np.matrix(odom[ANGULAR_T]):
            np.savetxt(f, line, fmt='%.2f')
    with open('TIME.txt','wb') as f:
        np.savetxt(f, imu[TIME], fmt='%.2f')
    with open('TWIST_COV.txt','wb') as f:
        np.savetxt(f, odom[TWIST_COV], fmt='%.2f')
    with open('POSE_COV.txt','wb') as f:
        np.savetxt(f, odom[POSE_COV], fmt='%.2f')
    with open('ORI_COV.txt','wb') as f:
        for line in np.matrix(imu[ORI_COV]):
            np.savetxt(f, line, fmt='%.2f')
