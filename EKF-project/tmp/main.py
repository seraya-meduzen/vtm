...
POSITION = 'position'
ORIENTATION = 'orientation'   #GET FROM ODOMETRY QUARTENION
LINEAR_A = 'linear_a'  #NOT USED
ANGULAR_V = 'angular_v' #NOT USED
LINEAR_T = 'linear_t'  #LINEAR VELOCITY
ANGULAR_T = 'angular_t' #ANGULAR  VELOCITY
POSE_COV = 'pose_cov'
TWIST_COV = 'twist_cov' #VELOCITY COV
ORI_COV = 'ori_cov'
TIME = 'time'
ODOM = '/odom'
IMU = '/imu/data'
...
def main():
    # -- Open Ros-bag file --
    db = RosDB("./telemetry_new.txt", True)
    ...
    # -- Get IMU data --
    roll = odom[ORIENTATION][:, 0]
    pitch = odom[ORIENTATION][:, 1]
    yaw = odom[ORIENTATION][:, 2]
    ...
    m_noise = np.matrix(odom[ORI_COV])
    ...
    # print([0, 0, 0, roll[0], pitch[0], yaw[0]])
    ...
    for i in range(100):
        ...
        tmPP = ekf_filter.predict(control)
